...
def get_cnn_model(model_num):
    ...
    l2 = list(itertools.product(*[feature_selector['l2_']].extend(general)))
    l2_clip = list(itertools.product(*[feature_selector['l2_clip_']].extend(general)))
    l2_models, model_num = convert_to_dict(l2, 'l2')
    l2_clip_models, model_num = convert_to_dict(l2_clip, 'l2_clip', model_counter)
    return l2_models + l2_clip_models

def convert_to_dict(grid, regularizer, model_num=0):
    list_of_models = []
    for model in grid:
        hyperparam_grid = {
    ...
    return list_of_models, model_num
